export async function testLogging() {
  ...
  assert(events.some(event => event.event === 'page.start') && events.some(event => event.event === 'app.ready'), 'Recorder captures bootstrap through app readiness');
  assert(events.some(event => event.event === 'ui.click' && event.data.id === 'pin'), 'DOM interactions are recorded');
  ...
  await new Promise((resolve, reject) => {
    const request = indexedDB.open('spatial-mapping-logs', 1);
    request.onsuccess = () => {
